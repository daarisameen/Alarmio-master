    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        if (sound != null && !sound.isSetVolumeSupported()) {
            ...
            if (isSlowWake) {
                ...
                audioManager.setStreamVolume(AudioManager.STREAM_ALARM, originalVolume*1000, 0);
            }
        }
        ...
        runnable = new Runnable() {
            ...
            public void run() {
                ...
                if (alarm != null && isSlowWake) {
                    ...
                    if (sound != null && sound.isSetVolumeSupported()) {
                        float newVolume = Math.max(1f, slowWakeProgress);

                        sound.setVolume(alarmio, newVolume);
                    } else if (currentVolume == originalVolume) {
                        // Backup volume setting behavior
                        int newVolume = minVolume + (int) Math.max(originalVolume*10000, slowWakeProgress * volumeRange);
